...
// Description: A class that properly resizes all the objects. This class also extends JApplet to draw. 
import javax.swing.*;
...
public class ScaledPoint extends JApplet
{
    ...
    //       FCTVAL = Width *(elementIndex / (2^treeHeight)
    {
        ...
        iterator = ((elementIndex+1) % (int) Math.pow(2, nodeLevel)); // Compute the node's index for its level, 
                                                                      //    which is the element's index modded with
       ...
       return position;                                               // return the positon 
    }
